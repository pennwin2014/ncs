...
if (bV == 1 && !mouseNode.navObj) mouseNode.navObj = doc.all["node" + mouseNode.id]
if (type == 0)
	if (mouseNode.isOpen)
		{
	...
	if (mouseNode.isFolder)
		if ((!mouseNode.isOpen && mouseOverIconMode == 1) || mouseOverIconMode == 2) timeoutIDOver = setTimeout("clickOnNode(" + folderId + ")",350)
	}
...
ret += "><B><CENTER>请等待<BR>目录树建造</B><P>"
ret += "<font size=-1>正在读取目录数据<br>"
initImage()
subret = "<img src='"
...
doc.write(ret)
if (doc.all) setTimeout("doc.close()",200)
else
{
doc.close()
...
if (document.all)
	...
	if (document.layers)
		{
		bV = 2
		if (inTable || inForm) bV = 0
		}
	else
		bV = 0
	}
if (evalVersion && navigator.userAgent.indexOf("Win") == -1) bV = 0
if (bV == 2 && !noFrame) self.onresize = self.handleResize
...
doc.write("<Title></Title><style type='text/css'>body {  font-size: 9pt}table {  font-size: 9pt}</style><link rel='stylesheet' href='../../css.css' type='text/css'></head>")
resizestr = ""
if (bV == 2 && !noFrame) resizestr = "onResize = 'parent.handleResize(event)' "
if (bV == 2) resizestr += " onLoad = 'parent.backLoad()' " 
doc.write("<BODY " + resizestr + bodyOption + " oncontextmenu='return false'>")
setStatus("Please wait for menu.")
}
initImage()
if (bV == 2 && !noFrame) self.onresize = self.handleResize
if (evalVersion)
	...
	if (bV == 2)
		{
		if (ftFolder == "")
			doc.write( "<APPLET CODE='EssfT.class' NAME = 'EssfT' width='1' height = '1'></APPLET>")
		else
			doc.write("<APPLET CODE='EssfT.class' CODEBASE = '" + ftFolder + "' NAME = 'EssfT' width='1' height = '1'></APPLET>")
		}
	}
if (bV == 2) 
	{
      if (noFrame) doc.write("<LAYER id = 'body' visibility = 'show' SRC= '" + noFrameBody + "'></LAYER>")
	...
	{
	if (bV == 1 && noFrame) doc.write("<TABLE border=0 style='height:100%;width:100%;'><TR><TD width = 1 valign = 'top'>")
	if (bV == 1 && noFrame) doc.write("<DIV id='foldertree' style='position:static;'>")
...
strbufarray = new Array
	...
	doc.write(menuFooter)
     doc.write("</DIV>")
	if (bV == 1 && noFrame) doc.write("</TD><TD><DIV><IFRAME id='body' visibility = 'show' SCROLLING = auto FRAMEBORDER = 0 BORDER = 0 SRC= '" + noFrameBody + "' style = 'position:relative;height:100%;width:100%;'></IFRAME></DIV></TD></TR></TABLE>")
	}
if (!noFrame){
doc.write("</BODY></HTML>")
if (bV == 2) doc.close()
...
addToTree = 0
clickAction = 0
pnIcon = "images/ftpn.gif";
mnIcon = "images/ftmn.gif";
pfnIcon = "images/ftpfn.gif";
mfnIcon = "images/ftmfn.gif";
plnIcon = "images/ftpln.gif";
mlnIcon = "images/ftmln.gif";
nIcon = "images/ftn.gif";
fnIcon = "images/ftfn.gif";
lnIcon = "images/ftln.gif";
bIcon = "images/ftb.gif";
vlIcon = "images/ftvl.gif";
