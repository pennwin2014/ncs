     ...
     help:function  (thisBtn) {
      ...
      var taskname = Ext.getCmp('taskname').value;
       ...
       return;}
         ...
         if(edate==''){
         Ext.Msg.alert('错误','结束时间不能为空');
         return;
       }
         var  servcount = Ext.getCmp('servcount').value;
         if(servcount ==''){
         Ext.Msg.alert('错误','重复出现个数不能为空');
         return;
       }
         var  maccount = Ext.getCmp('maccount').value;
         if(maccount ==''){
         Ext.Msg.alert('错误','伴随mac个数不能为空');
         return;
       }
      ...
      win.showAt(300,20);      
       ...
       adds:function(thisBtn){
         ...
         Ext.Msg.alert('任务名称不能为空');
         var taskdesc = Ext.getCmp('taskdesc').value;
         if(taskdesc=='')
         Ext.Msg.alert('任务描述不能为空');
         var  mac = Ext.getCmp('mac').value;
         if(mac=='')
         Ext.Msg.alert('MAC不能为空');
         var  dids = Ext.getCmp('groupname').value;
         if(dids=='')
         Ext.Msg.alert('场所不能为空');
         var  sdate=Ext.getCmp('sdate').value;
         if(sdate=='')
         Ext.Msg.alert('开始时间不能为空');
         var  edate=Ext.getCmp('edate').value;
         if(edate=='')
         Ext.Msg.alert('结束时间不能为空');
        
         var  servcount = Ext.getCmp('servcount').value;
         if(servcount =='')
         Ext.Msg.alert('重复出现个数不能为空');
         var  maccount = Ext.getCmp('maccount').value;
         if(maccount =='')
         Ext.Msg.alert('伴随mac个数不能为空');
         var startdate = new Date(sdate);
         var starttime = startdate.getTime()/1000;
         var enddate = new Date(edate);
          var endtime = enddate.getTime()/1000;
